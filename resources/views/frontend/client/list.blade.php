...
<div class="bg-gray p-4">
  <div class="d-flex justify-content-between align-items-center page-title-box">
    ...
    <div class="float-end"><span class="d-none d-lg-inline">Total Clients:</span> <span class="green-clr h4 ms-3">50</span></div>
  </div>
{{--  <div class="bg-light mt-4 dashboard-nav d-flex align-items-center ps-3 pe-3">--}}
{{--    <div class="ms-auto col-md-3 my-3">--}}
{{--      <form>--}}
{{--        <input class="w-100 p-1 pe-4 search-field" type="text" placeholder="Search by company name..." />--}}
{{--      </form>--}}
{{--    </div>--}}
{{--  </div>--}}
{{--  --}}
    ...
    <div>
        <a style="@if($company->total_allowed_clients_in_year == $company->total_clients_in_year)pointer-events: none;  @endif" href="{{ url('client/create')  }}" class="submit-btn">
            Create a New Client
        </a>
    </div>
    ...
    <table id="table_id">
      <thead>
        <tr class="no-bg">
          ...
          <th width="13%" scope="col">Forename</th>
          <th width="22%" scope="col">Email</th>
          <th width="13%" scope="col">Phone</th>
          <th width="19%" align="right" scope="col" class="text-end">Actions</th>
        </tr>
      </thead>
      <tbody>
      	<?php if($clients)
        ...
        <tr>
          ...
          <td data-label="Forename">{{ $client->first_name }}</td>
          <td data-label="Email">{{ $client->email }}</td>
          <td data-label="Phone">{{ $client->phone }}</td>
          <td data-label="Actions" class="text-end action-bar"><!-- <a href="#" class="table-btn view px-3 py-1">VIEW/SEND</a> --> <a href="{{url('client/edit'.'/'.$client->id)}}" class="table-btn edit p-1"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 15 15" fill="none">
            ...
            @if($client)
			@if($client->proposal == '' || $client->proposal->status == 'Not Sent' || $client->proposal->status == 'Rejected')
            <a  onclick="return confirm('Are you sure?')"
								href="{{$client->id}}" class="table-btn del p-1"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="13" viewBox="0 0 14 13" fill="none">
